...
describe(Movie) do
  ...
  describe("#name") do
    it("returns the name") do
      ...
      expect(movie.name()).to(eq("Oceans Eleven"))
    end
  end

  describe("#id") do
    it("returns the id") do
      movie = Movie.new({:name => "Oceans Eleven", :id => 1})
  ...
  describe "#update_actor_id" do
    it("lets you add an actor to a movie") do
      ...
      brad.save()
      movie.update_actor_ids({:actor_ids => [george.id(), brad.id()]})
      expect(movie.actors()).to(eq([george, brad]))
     end
   end

   describe("#update_name") do
     it("allows you to change the movie title") do
       movie = Movie.new({:name => "Oceans Eleven", :id => nil})
       movie.save()
       movie.update_name("Oceans Twelve")
